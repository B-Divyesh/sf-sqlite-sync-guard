...
- Web: `https://sqlite-sync-guard.sociobot.in/demo/` or `/?demo=1`. The query path immediately replaces itself with the same demo route and banner.
...
- CLI isolation: every run receives a unique `sqlite-sync-guard-demo-*` directory. The command prints its exact location and writes all sample output there. A clean-sandbox claim test checks that the current directory stays byte-for-byte unchanged.
